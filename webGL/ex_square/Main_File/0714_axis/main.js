...
class App {
	...
	_handleClick(event) {
        ...
        if (intersects.length > 0) {
            ...
            // Apply your desired lighting effect to the clicked object
            // For example, change its material or color


            // clickedObject.material.emissive.set(0xff0000); // Set emissive color to red
			
			// const clickedX = clickedObject.position.x;
			// const clickedY = clickedObject.position.y;
			// const clickedZ = clickedObject.position.z;
			// // Perform further actions with the clicked point's coordinates
			// 		console.log(`clicked Object: (${clickedX}, ${clickedY}, ${clickedZ})`);
	...
	_setupModel() {
		...
		const color_ig = ['#c96a64', '#a93e4f', '#e9e6c4', '#8a5a7f', '#aa3f50', '#a994c0', '#42507f', '#195d93']
		...
		for ( let ig = 0 ; ig < 8; ig ++) {
			...
			const material = new THREE.PointsMaterial({
				color: color_ig[ig], size: 2, sizeAttenuation: false })
			const point = new THREE.Points(geometry, material)

			const mNum = 1.5
			point.position.x = dx[ig] * mNum
			point.position.y = dy[ig] / 15
			point.position.z = dz[ig] * mNum
